...
export default createListModel({
  namespace: 'warningRulesassociatedDeviceModel',
  initState: {
    detail: {},
    currentFree: {
      page: 1,
  ...
  effects: {
    * getTableList({ payload }, { put, call, select }) {
      try {
        const { current,detail } = yield select(({ warningRulesassociatedDeviceModel }) => warningRulesassociatedDeviceModel)
        yield put({ type: 'setLoading', payload: true });
        const resdata = yield call(request.get, Urls.SpotCheckTemplate_getRelationList, { ...current, relationFlag: 3,earlyWarningRulesCode:payload.code })
    ...
    * getDeviceAlllist({ payload }, { put, call, select }) {
      try {
        const { currentFree, searchFieldsFree } = yield select(({ warningRulesassociatedDeviceModel }) => warningRulesassociatedDeviceModel)
        const resdata = yield call(request.get, Urls.device_list, { ...currentFree, ...searchFieldsFree, relationFlag: 3 })
