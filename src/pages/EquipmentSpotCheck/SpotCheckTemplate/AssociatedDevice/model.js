...
export default createListModel({
  namespace: 'associatedDeviceModel',
  initState: {
    detail: {},
    currentFree: {
      page: 1,
  ...
  effects: {
    * getTableList({ payload }, { put, call, select }) {
      try {
        const { current,detail } = yield select(({ associatedDeviceModel }) => associatedDeviceModel)
        yield put({ type: 'setLoading', payload: true });
        const resdata = yield call(request.get, Urls.SpotCheckTemplate_getRelationList, { ...current, relationFlag: 1,spotCheckTemplateCode:detail.templateCode })
    ...
    * getDeviceAlllist({ payload }, { put, call, select }) {
      try {
        const { currentFree, searchFieldsFree } = yield select(({ associatedDeviceModel }) => associatedDeviceModel)
        const resdata = yield call(request.get, Urls.device_list, { ...currentFree, ...searchFieldsFree, relationFlag: 1 })
